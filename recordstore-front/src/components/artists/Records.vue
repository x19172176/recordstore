  ...
  <div class="max-w-md m-auto py-10">
    ...
    <form action="" @submit.prevent="addRecord">
      ...
      <div class="mb-6">
        ...
        <p class="pt-4">Don't see an artist? <router-link class="text-gray-600 underline" to="/artists">Create one</router-link></p>
       </div>

      <input type="submit" value="Add Record" class="font-sans font-bold px-4 rounded cursor-pointer no-underline bg-green-200 hover:bg-green-dark-600 block w-full py-4 text-white items-center justify-center" />
    </form>
    ...
    <ul class="list-reset mt-4">
      <li class="py-4" v-for="record in records" :key="record.id" :record="record">
        ...
        <div class="flex items-center justify-between flex-wrap">
          ...
          <button class="bg-transparent text-sm hover:bg-blue-200 hover:text-white text-blue-200 border border-blue-200 no-underline font-bold py-2 px-4 mr-2 rounded"
          @click.prevent="editRecord(record)">Edit</button>

          <button class="bg-transparent text-sm hover:bg-red text-red-200 hover:text-white no-underline font-bold py-2 px-4 rounded border border-red-200"
        ...
        <div v-if="record == editedRecord">
          <form action="" @submit.prevent="updateRecord(record)">
            <div class="mb-6 p-4 bg-white rounded border border-gray-300 mt-4">
              ...
              <input type="submit" value="Update" class="bg-transparent text-sm hover:bg-blue-200 hover:text-white text-blue-200 border border-blue-200 no-underline font-bold py-2 px-4 mr-2 rounded">
            </div>
          </form>
        </div>
      </li>
